...
import { mockTrustService } from '../services/mockService';
...
router.get('/', extractToken, async (req: AuthenticatedRequest, res: Response) => {
  if (config.mockMode) {
    return res.json({ ...mockTrustService, mock: true });
  }
  if (!req.userToken) return res.status(401).json({ error: 'Unauthorized' });
  try {
    const data = await iamService.getTrustService(req.userToken);
    return res.json(data);
  } catch (err: unknown) {
    const error = err as { response?: { status: number; data: unknown } };
    return res.status(error?.response?.status || 500).json({
      error: 'Failed to fetch trust service data',
